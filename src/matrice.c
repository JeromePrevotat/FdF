...
void	apply_matrice(double **matrice, t_3D_p_tab *p_tab)
{
	...
	while (y < p_tab->y_max)
	{
		...
		{
			w = 1;
			p_tab->tab[y][x].x = round(p_tab->tab[y][x].x * matrice[0][0]
				+ p_tab->tab[y][x].x * matrice[0][1] + p_tab->tab[y][x].x
				* matrice[0][2] + p_tab->tab[y][x].x * matrice[0][3]);
			p_tab->tab[y][x].y = round(p_tab->tab[y][x].y * matrice[1][0]
				+ p_tab->tab[y][x].y * matrice[1][1] + p_tab->tab[y][x].y
				* matrice[1][2] + p_tab->tab[y][x].y * matrice[1][3]);
			p_tab->tab[y][x].z = round(p_tab->tab[y][x].z * matrice[2][0]
				+ p_tab->tab[y][x].z * matrice[2][1] + p_tab->tab[y][x].z
				* matrice[2][2] + p_tab->tab[y][x].z * matrice[2][3]);
			w = round(w * matrice[3][0] + w * matrice[3][1] + w * matrice[3][2]
				+ w * matrice[3][3]);
			if (w != 1 && w != 0)
			{
