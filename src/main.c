	...
	if (argc == 2)
	{
		...
		*point_list = NULL;
		if (!(p_tab = (t_3d_p_tab *)malloc(1 * sizeof(t_3d_p_tab))))
			return (ERROR);
		init_all(point_list, p_tab, argc, argv);
		start_drawing(p_tab, point_list, argv[1]);
	}
	else
		usage();
	return (0);
...
void	init_all(t_3d_p_list **point_list, t_3d_p_tab *p_tab, int argc,
		char **argv)
{
	if (parsing_file(argv[1], point_list) != 1)
		error();
	if (!(p_tab->tab = (t_3d_point **)malloc(get_tab_y(point_list) *
		sizeof(t_3d_point *))))
		error();
	argc = 0;
	while (argc != get_tab_y(point_list))
	{
		if (!(p_tab->tab[argc] = (t_3d_point *)malloc(
			get_tab_x(point_list) * sizeof(t_3d_point))))
			error();
		argc++;
	}
...
void	start_drawing(t_3d_p_tab *p_tab, t_3d_p_list **point_list, char *title)
{
	t_window	*win1;
	void		*mlx;
	t_3d_p_tab	*base_tab;
	...
	mlx = mlx_init();
	win1 = init_window(mlx, title, 800, p_tab);
